...
export default function Home() {
  ...
  if (!apiKey || showKeyScreen) {
    return (
      <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0a0a0a,#111827)", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Manrope',sans-serif", padding:"24px" }}>
        <style>{`@import url('https://fonts.googleapis.com/css2?family=Manrope:wght@400;500;600;700;800&display=swap');`}</style>
        <div style={{ width:"100%", maxWidth:"400px", background:"#111", borderRadius:"24px", padding:"32px", border:"1px solid rgba(255,255,255,0.08)" }}>
          <div style={{ width:"56px", height:"56px", borderRadius:"16px", background:"linear-gradient(135deg,#ff8c00,#ff6b00)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"26px", marginBottom:"24px" }}>🤖</div>
          <div style={{ fontWeight:800, fontSize:"22px", color:"#fff", marginBottom:"8px" }}>AI Assistant</div>
          <div style={{ fontSize:"14px", color:"rgba(255,255,255,0.4)", marginBottom:"28px", lineHeight:"1.6" }}>Введіть Anthropic API ключ щоб запустити.</div>
          <div style={{ fontSize:"12px", color:"rgba(255,255,255,0.4)", marginBottom:"8px", fontWeight:600, letterSpacing:"1px" }}>API КЛЮЧ</div>
          <input type="password" value={apiKeyInput} onChange={e=>setApiKeyInput(e.target.value)}
            onKeyDown={e=>{ if(e.key==="Enter"&&apiKeyInput.startsWith("sk-")){setApiKey(apiKeyInput);setShowKeyScreen(false);}}}
            placeholder="sk-ant-..." style={{ width:"100%", padding:"14px 16px", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:"12px", color:"#fff", fontSize:"14px", fontFamily:"inherit", outline:"none", boxSizing:"border-box", marginBottom:"16px" }}/>
          <button onClick={()=>{ if(apiKeyInput.startsWith("sk-")){setApiKey(apiKeyInput);setShowKeyScreen(false);}}}
            style={{ width:"100%", padding:"14px", background:apiKeyInput.startsWith("sk-")?"linear-gradient(135deg,#ff8c00,#ff6b00)":"rgba(255,255,255,0.06)", border:"none", borderRadius:"12px", color:"#fff", fontSize:"15px", fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
            Запустити →
          </button>
          <div style={{ fontSize:"11px", color:"rgba(255,255,255,0.25)", marginTop:"16px", textAlign:"center" }}>console.anthropic.com → API Keys</div>
        </div>
      </div>
    );
  }
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0a0a0a,#111827)", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Manrope',sans-serif", padding:"16px" }}>
      ...
      <div style={{ width:"100%", maxWidth:"480px", display:"flex", flexDirection:"column", height:"90vh", maxHeight:"700px", background:"#111", borderRadius:"24px", overflow:"hidden", border:"1px solid rgba(255,255,255,0.08)", boxShadow:"0 32px 80px rgba(0,0,0,0.6)" }}>
        <div style={{ padding:"20px 24px", background:"linear-gradient(135deg,#1a1a1a,#222)", borderBottom:"1px solid rgba(255,255,255,0.06)", display:"flex", alignItems:"center", gap:"14px" }}>
          ...
          <button onClick={()=>{setShowKeyScreen(true);setApiKeyInput(apiKey);}} style={{ marginLeft:"auto", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:"8px", color:"rgba(255,255,255,0.4)", fontSize:"11px", padding:"4px 10px", cursor:"pointer", fontFamily:"inherit" }}>🔑 ключ</button>
        </div>
